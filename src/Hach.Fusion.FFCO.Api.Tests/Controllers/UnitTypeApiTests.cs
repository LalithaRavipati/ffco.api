...
using Hach.Fusion.D.Seed;
using Moq;
using UnitTypeFacade = Hach.Fusion.FFCO.Business.Facades.UnitTypeFacade;

namespace Hach.Fusion.FFCO.Api.Tests.Controllers
{
    [TestFixture]
    public class UnitTypeApiTests
    {

        private UnitTypesController _controller;
        ...
        public UnitTypeApiTests()
        {
            ...
            _mockDtoOptions = new Mock<ODataQueryOptions<UnitTypeQueryDto>>(
                new ODataQueryContext(builder.GetEdmModel(), typeof(UnitTypeQueryDto), new ODataPath()),
                request);
